...
En este enfoque, el Feedforward se suma a la salida del controlador maestro (LIC). Dado que el Setpoint del lazo de flujo (FIC) debe estar en unidades de caudal ($m^3/s$ o $L/min$), la ecuación de prealimentación \textbf{no debe incluir la ganancia de la bomba ($k_1$)}. 
...
Por esto, se utilzará la arquitectura 1, siendo esta más segura y simple de implementear, siguiendo la filosofía \textbf{KISS} (\textit{keep it simple, stupid}).

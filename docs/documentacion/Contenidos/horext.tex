...
\begin{itemize}
    \item \textbf{Lado A} (dinámica de salida): $n$ parámetros $\bar{\alpha}_0, \ldots, \bar{\alpha}_{n-1}$
    \item \textbf{Lado B} (respuesta a controles): $n + T - 1$ parámetros $\bar{\beta}_1, \ldots, \bar{\beta}_{n+T-1}$
    \item \textbf{Total}: $n + (n + T - 1) = 2n + T - 1$ parámetros
\end{itemize}

Es importante destacar que estos coeficientes \textbf{no corresponden} a los polinomios $A$ y $B$ del sistema físico. Son combinaciones lineales de los parámetros reales convolucionados con los polinomios auxiliares $F$ y $G$ de la identidad polinomial de Ydstie (Ec. 3 del paper). El estimador identifica directamente el modelo de predicción a $T$ pasos, no el modelo del sistema.
...
Bajo la \textbf{Estrategia 1} (control constante), se asume que los incrementos de control futuros son nulos: $\Delta u_{k+1} = \Delta u_{k+2} = \ldots = \Delta u_{k+T-1} = 0$. El regresor en el instante $k$ queda:
...
\[
    ...
    \oplus
    \underbrace{\begin{bmatrix}
        0 \\ \vdots \\ 0 \\ \Delta u_k \\ \vdots \\ \Delta u_{k-n+1}
...
\[
    ...
    \qquad
    \bar{\phi}_k = \begin{bmatrix} \Delta y_k \\ 0 \\ 0 \\ \Delta u_k \end{bmatrix}_{4\times1}
\]
...
\[
    ...
    \begin{bmatrix}
        ...
        \underbrace{0,\ \ldots,\ 0}_{T-1},\
        \Delta u_k,\ \ldots,\ \Delta u_{k-n+1}
    \end{bmatrix}^T
\]

Nótese que $\Delta u_k$ es el esfuerzo de control calculado en el ciclo anterior y ya aplicado a la planta.

\paragraph{Paso 2: Error de predicción (innovación)}

El error que corrige al estimador en el instante $k$ es el que se cometió hace $T$ pasos. Recién en $k$ se dispone de la medición real para comparar contra aquella predicción. Por esto el RLS utiliza el regresor retrasado $\bar{\phi}_{k-T}$, que debe mantenerse en un buffer circular de longitud $T$:
...
El factor de normalización escalar $D$ se calcula con el regresor retrasado:
\[
...
\[
    \lambda_k = 1 - \frac{e_k^2}{\Sigma_0 \cdot D}, \qquad \lambda_k \in [0.95,\ 1.00]
\]
...
\[
    ...
    + \sum_{i=1}^{n} \bar{\beta}_{T+i-1}\, \Delta u_{k-i+1}
\]
...
\textbf{Caso concreto} ($n=1$, $T=3$):

\[
    \bar{h}_{k+3} = y_k + \bar{\alpha}_0\,\Delta y_k + \bar{\beta}_3\,\Delta u_k
\]
\[
...
\[
    \Delta u_{k-n} \leftarrow \cdots \leftarrow \Delta u_{k-1} \leftarrow \Delta u_k
\]
